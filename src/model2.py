...
class TFParts(object):
    ...
    def build(self):
        ...
        with tf.compat.v1.variable_scope("graph"):
            # Variables (matrix of embeddings/transformations)
            # KG1
            self._ht1 = ht1 = tf.compat.v1.get_variable(
                name='ht1',  # for t AND h
                shape=[self._num_entsA, self._dim1],
                dtype=tf.float32)
            self._r1 = r1 = tf.compat.v1.get_variable(
                name='r1',
                shape=[self._num_relsA, self._dim1],
                dtype=tf.float32)
            # KG2
            self._ht2 = ht2 = tf.compat.v1.get_variable(
                name='ht2',  # for t AND h
                shape=[self._num_entsB, self._dim2],
                dtype=tf.float32)
            self._r2 = r2 = tf.compat.v1.get_variable(
                name='r2',
                shape=[self._num_relsB, self._dim2],
                dtype=tf.float32)

            tf.compat.v1.summary.histogram("ht1", ht1)
            tf.compat.v1.summary.histogram("ht2", ht2)
            tf.compat.v1.summary.histogram("r1", r1)
            tf.compat.v1.summary.histogram("r2", r2)
            ...
            self._A_h_index = A_h_index = tf.compat.v1.placeholder(
                ...
                name='A_h_index')
            self._A_r_index = A_r_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK1],
                name='A_r_index')
            self._A_t_index = A_t_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK1],
                name='A_t_index')
            self._A_hn_index = A_hn_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK1],
                name='A_hn_index')
            self._A_tn_index = A_tn_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK1],
                name='A_tn_index')
            '''
            ...
            self._B_h_index = B_h_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK2],
                name='B_h_index')
            self._B_r_index = B_r_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK2],
                name='B_r_index')
            self._B_t_index = B_t_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK2],
                name='B_t_index')
            self._B_hn_index = B_hn_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK2],
                name='B_hn_index')
            self._B_tn_index = B_tn_index = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeK2],
                name='B_tn_index')
            ...
            ######################## Type Loss #######################
            self._AM_index1 = AM_index1 = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeA],
                name='AM_index1')
            self._AM_index2 = AM_index2 = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeA],
                name='AM_index2')
            
            self._AM_nindex1 = AM_nindex1 = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeA],
                name='AM_nindex1')
            self._AM_nindex2 = AM_nindex2 = tf.compat.v1.placeholder(
                dtype=tf.int64,
                shape=[self._batch_sizeA],
                name='AM_nindex2')
            ...
            self._bc = b_c = tf.compat.v1.get_variable(name='bc', shape=[self._hidden_dim],initializer=tf.truncated_normal_initializer,dtype=tf.float32)
            self._Me = Me = tf.compat.v1.get_variable(name='Me', shape=[self._dim1, self._hidden_dim],initializer=orthogonal_initializer(),dtype=tf.float32)
            self._be = b_e = tf.compat.v1.get_variable(name='be', shape=[self._hidden_dim],initializer=tf.truncated_normal_initializer,dtype=tf.float32)
